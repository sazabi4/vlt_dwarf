...
def compute_heliocorr_lco(filename): 
    # subroutine to compute the heliocentric correction for Magellan/IMACS data
    # example:
    ## import heliocorr
    ## vcorr = heliocorr.compute_heliocorr_lco(input_fits_file)
    hdr=pyfits.open(filename)[0].header
    obst=hdr['DATE-OBS']+'T'+hdr['TIME-OBS']
    t = Time(obst, format = 'isot', scale = 'utc')
    corr = x_keckhelio(hdr['RA-D'], hdr['DEC-D'], jd = t.jd, obs='lco')
    return -1*corr[0]

def compute_heliocorr_vlt(filename):
    # subroutine to compute the heliocentric correction for VLT/GIRAFFE data
    # example:
    ## import heliocorr
    ## vcorr = heliocorr.compute_heliocorr_vlt(input_fits_file)
    hdr=pyfits.open(filename)[0].header
    obst=hdr['DATE-OBS']
    t = Time(obst, format = 'isot', scale = 'utc')
    corr = x_keckhelio(hdr['RA'], hdr['DEC'], jd = t.jd, obs='vlt')
    return -1*corr[0]

def compute_heliocorr_aat(filename):
    # subroutine to compute the heliocentric correction for VLT/GIRAFFE data
    # example:
    ## import heliocorr
    ## vcorr = heliocorr.compute_heliocorr_vlt(input_fits_file)
    hdr=pyfits.open(filename)[0].header
    obst=hdr['UTMJD']
    t = Time(obst, format = 'mjd', scale = 'utc')
    corr = x_keckhelio(hdr['MEANRA'], hdr['MEANDEC'], jd = t.jd, obs='aat')
    return -1*corr[0]
...
def x_keckhelio(ra, dec, epoch=2000.0, jd=None, tai=None,
    ...
    if longitude is not None and latitude is not None and altitude is not None:
        ...
        longitude = 360. - 70.6926
        latitude = -29.0146
        altitude = 2282.      #meters
    elif obs == 'aat':
        longitude = 149.0661  #degree
        latitude = -31.27704   #degree
